...
train_data = torch.tensor(encode(text), dtype=torch.long).to(device)
...
class Model(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        position_mbed = self.position_embedding(torch.arange(T, device=device))
...
for steps in tqdm(range(500000), desc="Training process:"):
    ix = torch.randint(len(train_data) - block_size, (batch_size,)).to(device)
